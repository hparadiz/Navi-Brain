...
use NaviBrain\Core\DecisionStateMachine\Dependencies;
...
class DecisionStateMachine
{
    ...
    public function start(int $intentionId, string $trigger, ?int $threadId = null, ?string $modelHint = null, ?int $parentRunId = null): array {
        ...
        foreach (['running', 'waiting'] as $activeStatus) {
            $active = DecisionCycle::getAllByWhere(['intention_id' => $intentionId, 'status' => $activeStatus], ['order' => ['id' => 'ASC'], 'limit' => 1]);
            if ($active !== []) {
                throw new RuntimeException(sprintf( 'Intention %d already has active decision cycle %d.', $intentionId, (int) $active[0]->id ));
            }
        }
        ...
        $intentionIds = array_merge([$intentionId], (array) $intention->dependencies, $intention->parent_id === null ? [] : [(int) $intention->parent_id]);
        foreach (array_unique($intentionIds) as $id) {
            Dependencies::captureDependency($inputSnapshot, 'intention', (int) $id, (int) $id === $intentionId ? $intention->getData() : null);
        }

        $working = $this->core->workingMemory()->snapshot($threadId);
        if ($this->core->otherModel()->isAblated()) {
            $working = array_values(array_filter( $working, static fn (array $slot): bool => ($slot['slot_role'] ?? null) !== 'other_agent_state' ));
        }
        $workingJson = Dependencies::encode($working);
        $cycle = $this->core->beginDecisionPlanning($intentionId, $threadId, $trigger, $modelHint, hash('sha256', $workingJson));
        try {
            $result = $this->planStartedCycle($cycle, $intention, (object) [ 'inputSnapshot' => $inputSnapshot, 'working' => $working, 'modelHint' => $modelHint, 'parentRunId' => $parentRunId, ]);
        } catch (Throwable $throwable) {
            $failed = $this->core->finishDecisionPlanning((int) $cycle->id, 'Decision planning failed: ' . $throwable->getMessage());
            if ($failed !== null) {
                return $failed;
            }
     ...
     * @return array<string, mixed>
     */
    private function planStartedCycle(DecisionCycle $cycle, Intention $intention, \stdClass $context): array {
        ...
        $observations = [
            ...
            'working_memory_roles' => array_values(array_map( static fn (array $slot): string => (string) ($slot['slot_role'] ?? ''), $context->working )),
            'other_agent_state' => array_values(array_filter( $context->working, static fn (array $slot): bool => ($slot['slot_role'] ?? null) === 'other_agent_state' ))[0] ?? null,
            'other_agent_model_ablated' => $this->core->otherModel()->isAblated(),
        ];
        foreach ($pending as $event) {
            Dependencies::captureDependency($context->inputSnapshot, 'sense_event', (int) ($event['id'] ?? 0), $event);
        }
        $cycle->setField('observations', $observations + ['input_snapshot' => $context->inputSnapshot]);
        $this->advance($cycle, 'retrieve', 'observe', $this->elapsedMs($started));

        $started = hrtime(true);
        $query = implode(' ', [ (string) $intention->title, (string) $intention->next_action, $cycle->trigger, ]);

        try {
            $retrievalAction = $this->core->proceduralMemory()->executeAction(
                new ActionTrace(['intention_id' => $intentionId, 'description' => 'Retrieve long-term memory relevant to the active decision.', 'expected' => 'A bounded, machine-verified result set is returned.'], true, true), new \NaviBrain\Model\ActionExecution(['action_kind' => 'memory.search', 'arguments' => ['query' => $query, 'limit' => 8]], true, true)
            );
        ...
        if (($retrievalAction['status'] ?? null) === 'held') {
            ...
            $this->core->rejectPendingActionExecution($actionId, [ 'error' => 'Planning retrieval was held by cognition pause; start a fresh cycle.', 'dispatch_not_attempted' => true, ]);
        ...
        foreach ((array) ($retrievalObserved['memories'] ?? []) as $memory) {
            ...
            Dependencies::captureDependency($context->inputSnapshot, 'memory', (int) ($memory['id'] ?? 0), $memory);
            $retrieved[] = [
                'id' => (int) ($memory['id'] ?? 0),
        ...
        if ($retrieved !== []) {
            ...
            $this->core->workingMemory()->publish(new WorkingMemorySlot([
                'slot_role' => 'supporting_evidence',
                'claim' => (string) ($first['content'] ?? ''),
                'record_type' => 'memory',
                'record_id' => (int) ($first['id'] ?? 0),
                'confidence' => (float) ($first['confidence'] ?? 0.5),
                'scope_key' => $threadId === null ? 'shared' : 'thread:' . $threadId,
                'thread_id' => $threadId,
            ], true, true));
        }
        ...
        $prompt = implode("\n", [
            ...
            "Intention:\n" . PlainText::render([ 'title' => (string) $intention->title, 'next_action' => (string) $intention->next_action, 'success_condition' => (string) $intention->success_condition, ], 3000, 8),
            'Trigger: ' . PlainText::sanitize($cycle->trigger),
        ...
        $queued = $this->core->enqueueWork(
            new \NaviBrain\Model\WorkItem([
                ...
                'parent_intention_id' => $intentionId,
                ...
                'model_hint' => $context->modelHint,
            ],
                'token_budget' => 768,
                'wall_budget_seconds' => 300,
                'idempotency_key' => 'decision_cycle:' . (int) $cycle->id . ':proposal',
                'allowed_actions' => array_values(array_map( static fn (array $adapter): string => (string) $adapter['action_kind'], $adapters ))
            ], true, true)
        );
        ...
        if ($cycle->status === 'waiting' && $cycle->state === 'reason') {
            ...
            $timingRecord = DecisionCycle::getByWhere(['id' => (int) $cycle->id, 'proposal_work_item_id' => (int) ($queued['work_item']['id'] ?? 0), 'status = \'waiting\'', 'state = \'reason\'']);
            if ($timingRecord instanceof DecisionCycle) {
                $timingRecord->setFields([ 'stage_timings' => $timings ]);
                $timingRecord->save();
            }
            $cycle = DecisionCycle::getByID((int) $cycle->id);
        }
    ...
    public function integrate(array $work, array $proposal, ?string $model): array
    {
        ...
        try {
            ...
            $failed = $this->core->finishDecisionIntegration($cycleId, (int) $work['id'], 'Decision integration failed: ' . $throwable->getMessage());
            if ($failed !== null) {
                return $failed;
            }
    ...
    private function integrateClaimed(DecisionCycle $cycle, array $work, array $proposal): array
    {
        ...
        $createdAt = $workRecord instanceof WorkItem ? Dependencies::timestamp($workRecord->created_at) : null;
        $timings = is_array($cycle->stage_timings) ? $cycle->stage_timings : [];
        ...
        try {
            ...
            return $this->fail($durable instanceof DecisionCycle ? $durable : $cycle, 'Reasoning publication rejected: ' . $throwable->getMessage());
        }
        $cycle->setField('reasoning', [
            ...
            'working_memory_roles_written' => array_values(array_filter([ $currentPlan === '' ? null : 'current_plan', $decisionBasis === '' ? null : 'decision_basis', ])),
        ]);
        ...
        foreach ($candidates as $index => $candidate) {
            ...
            $inspection = $this->core->proceduralMemory()->inspectCandidate($kind, $arguments, $description, $expected);
            ...
            $sourceWasRetrieved = $actionClass !== 'learning'
                || in_array((int) ($arguments['episode_id'] ?? 0), array_map('intval', (array) ($retrievalState['memory_ids'] ?? [])), true);
            ...
            $grounding = $this->groundingScore($description . ' ' . $rationale, (string) $cycle->trigger . ' ' . $this->retrievalText($cycle));
            $score = $valid
                ? ($baseConfidence * 0.25)
                    + 0.2
        ...
        $started = hrtime(true);
        $valid = array_values(array_filter( $stored, static fn (array $entry): bool => $entry['row']->status === 'proposed' ));
        ...
        try {
            $outcome = $this->core->proceduralMemory()->executeAction(
                new ActionTrace(['intention_id' => (int) $cycle->intention_id, 'description' => (string) $selected->description, 'expected' => (string) $selected->expected], true, true), new \NaviBrain\Model\ActionExecution(['action_kind' => (string) $selected->action_kind, 'arguments' => is_array($selected->arguments) ? $selected->arguments : [], 'procedure_id' => isset($inspection['procedure']['procedure_id'])
                    ? (int) $inspection['procedure']['procedure_id']
                    : null, 'procedure_memory_id' => isset($inspection['procedure']['memory_id'])
                    ? (int) $inspection['procedure']['memory_id']
                    : null], true, true), $cycle
            );
        } catch (Throwable $throwable) {
        ...
        if ($actionExecution instanceof \NaviBrain\Model\ActionExecution
            ...
            $finished = is_array($outcome['finished'] ?? null)
                ? $outcome['finished']
                : $this->core->proceduralMemory()->finishDurableAction($actionId);
            return $this->completeAsyncAction($actionId, (string) $actionExecution->status === 'succeeded' && (int) $actionExecution->verified === 1, $finished);
        }
    ...
    public function completeAsyncAction(int $actionId, bool $matched, array $finished): array
    {
        ...
        if ($cycleId === null) {
            ...
            foreach (DecisionCycle::getAllByWhere(['status' => 'waiting', 'state' => 'verify'], ['order' => ['id' => 'ASC']]) as $cycle) {
                ...
                $requestEventId = $actionExecution instanceof \NaviBrain\Model\ActionExecution
                    ...
                    : 0;
                $this->core->bindLegacyDecisionAsyncClaim((int) $cycle->id, $actionId, $requestEventId);
                $cycleId = (int) $cycle->id;
                break;
            }
        }
            ...
            try {
                ...
                if ($execution->status === 'pending') {
                    if (!in_array($claim['status'], ['started', 'held'], true)
                        || $this->core->rejectSettledDecisionPendingAction((int) $claim['decision_cycle_id'], $actionId) === null) {
                        continue;
                    }
                    $execution = \NaviBrain\Model\ActionExecution::getByField('action_trace_id', $actionId);
                }
        ...
        foreach ($this->core->pendingDecisionActionClaims($limit) as $claim) {
            ...
            if ((string) $execution->status === 'pending') {
                ...
                if (in_array($claim['status'], ['started', 'held'], true)
                    ...
                    if ($this->core->rejectSettledDecisionPendingAction((int) $claim['decision_cycle_id'], $actionId) === null) {
                        continue;
                    }
                } else {
            ...
            $finished = $this->core->proceduralMemory()->finishDurableAction($actionId);
            $reconciled[] = $this->completeAsyncAction($actionId, (string) $execution->status === 'succeeded' && (int) $execution->verified === 1, $finished);
        }

        foreach (DecisionCycle::getAllByWhere(['status' => 'waiting', 'state' => 'verify'], ['order' => ['id' => 'ASC'], 'limit' => max(1, min(512, $limit))]) as $cycle) {
            ...
            $this->core->bindLegacyDecisionAsyncClaim((int) $cycle->id, $actionId, (int) $execution->dispatch_event_id);
            $finished = $this->core->proceduralMemory()->finishDurableAction($actionId);
            $reconciled[] = $this->completeAsyncAction($actionId, (string) $execution->status === 'succeeded' && (int) $execution->verified === 1, $finished);
        }
        return $reconciled;
    ...
    public function failWork(array $work, string $error): array
    {
        ...
        $statementRecord = DecisionCycle::getByWhere(['id' => $cycleId, 'proposal_work_item_id' => (int) ($work['id'] ?? 0), 'status = \'waiting\'', 'state = \'reason\'']);
        if ($statementRecord instanceof DecisionCycle) {
            $statementRecord->setFields([ 'state' => 'failed', 'status' => 'failed', 'error' => $error, 'completed_at' => $now, 'updated_at' => $now ]);
            $statementRecord->save();
        }
        $failed = ($statementRecord instanceof DecisionCycle ? 1 : 0) === 1;
        $cycle = DecisionCycle::getByID($cycleId);
        if (!$cycle instanceof DecisionCycle) {
        ...
        $event = $this->core->emitEvent('decision_cycle.failed', [ 'decision_cycle_id' => $cycleId, 'failed_state' => 'reason', 'model_id' => $cycle->model_id, 'error' => $error, ]);
    ...
    public function show(int $id): array
    {
        ...
        $candidates = DecisionCandidate::getAllByWhere(['decision_cycle_id' => $id], ['order' => ['rank' => 'ASC']]);
        return [
            'cycle' => $cycle->getData(),
            'candidates' => array_values(array_map( static fn (DecisionCandidate $candidate): array => $candidate->getData(), $candidates )),
        ];
    }
    ...
    public function prepareWorkspaceDependencies(int $cycleId, array &$workspace, array $memoryObservations = []): array
    {
        return Dependencies::prepareWorkspaceDependencies($cycleId, $workspace, $memoryObservations);
    }

    public function decisionInputChanges(int $cycleId): array
    {
        return Dependencies::decisionInputChanges($cycleId);
    }
    ...
    private function fail(DecisionCycle $cycle, string $error): array
    {
        ...
        $cycle->save();
        $event = $this->core->emitEvent('decision_cycle.failed', [ 'decision_cycle_id' => (int) $cycle->id, 'failed_state' => $failedState, 'model_id' => $cycle->model_id, 'error' => $error, ]);
        return ['status' => 'failed', 'cycle' => $cycle->getData(), 'event' => $event];
    }
    ...
    {
        $now = time();
        $cycle->setFields([ 'state' => 'impasse', 'status' => 'impasse', 'error' => $reason, 'completed_at' => $now, 'updated_at' => $now, ]);
        $cycle->save();
        $event = $this->core->emitEvent('decision_cycle.impasse', [ 'decision_cycle_id' => (int) $cycle->id, 'model_id' => $cycle->model_id, 'reason' => $reason, 'reconsider_on_next_cycle' => true, ]);
    ...
    private function advance(DecisionCycle $cycle, string $next, string $completed, int $elapsedMs): void
    {
        ...
        $cycle->save();
        $this->core->emitEvent('decision_cycle.transition', [ 'decision_cycle_id' => (int) $cycle->id, 'completed_state' => $completed, 'next_state' => $next, 'elapsed_ms' => $elapsedMs, ]);
    }

    private function retrievalText(DecisionCycle $cycle): string
    {
        $retrieval = is_array($cycle->retrieval) ? $cycle->retrieval : [];
        return implode(' ', array_map( static fn (array $memory): string => (string) ($memory['content'] ?? ''), array_filter((array) ($retrieval['memories'] ?? []), 'is_array') ));
    }

    private function groundingScore(string $candidate, string $context): float
    {
        $words = static fn (string $text): array => array_values(array_unique(array_filter( preg_split('/[^\p{L}\p{N}]+/u', mb_strtolower($text)) ?: [], static fn (string $word): bool => mb_strlen($word) > 3 )));
